...
from config import GIS_FEATURE_SERVICE_AUTHENTICATION, GIS_FEATURE_SERVICE
...
class GISService:
    ...
    def _get_feature_service_token(self):
        data = {
            ...
            'password': get_secret(os.environ['PROJECT_ID'], GIS_FEATURE_SERVICE_AUTHENTICATION['secret']),
            'request': GIS_FEATURE_SERVICE_AUTHENTICATION['request'],
            'referer': GIS_FEATURE_SERVICE_AUTHENTICATION['referer'],
        }

        data = self.requests_session.post(GIS_FEATURE_SERVICE_AUTHENTICATION['url'], data).json()
        return data['token']

    def add_object_to_feature_layer(self, gis_object):
        data = {
            'adds': json.dumps([gis_object]),
            'f': 'json',
            'token': self.token
        }
        ...
        try:
            ...
            if response.get('error', False):
                raise Exception(f"Error when pushing to GIS server - "
