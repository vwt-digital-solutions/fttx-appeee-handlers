...
class ImageService:
    ...
    def _compose_image_store_base_path(self, form_entry):
        if constant.PROVIDER_ID_KEY in form_entry and constant.ENTRY_KEY in form_entry:
            ...
            if form_code:
                image_store_base_path = f"{config.IMAGE_STORE_PATH}/{provider_id}/{form_code}/{ds_row_id}"
    ...
    def _compose_image_download_base_url(self, form_entry):
        if constant.PROVIDER_ID_KEY in form_entry and constant.ENTRY_KEY in form_entry:
            ...
            ds_row_id = form_entry[constant.ENTRY_KEY].get(constant.DS_ROW_ID_KEY)
            if ds_row_id:
                image_download_base_url = f"{config.IMAGE_DOWNLOAD_BASE_URL}/{provider_id}-{ds_row_id}"
                logging.info(f"Image download base url is {image_download_base_url}")
                return image_download_base_url

        raise Exception(f"Error composing base_url, {constant.PROVIDER_ID_KEY} or {constant.DS_ROW_ID_KEY} missing")
        ...
        for page_k, page_v in pages.items():
            for answer_k, answer_v in page_v.items():
                if self._is_image_to_download(answer_k, answer_v):
                    logging.info(f"Answer on {answer_k} is an image: {answer_v}, start downloading")
                    ...
                    if file_response.status_code == requests.codes.ok:
                        file_store_path = f"{image_store_base_path}/{answer_k}_{answer_v}"
                        content_type, _ = guess_type(file_store_path)
                        if storage.Blob(bucket=image_store_bucket, name=file_store_path).exists(self.storage_client):
                            logging.info(f"Skipping file {file_store_path}. It already exists.")
                        else:
                            logging.info(f"Storing {file_store_path} as {content_type}")
                            image_store_blob = image_store_bucket.blob(file_store_path)
                            image_store_blob.upload_from_string(file_response.content, content_type=content_type)

                        form_entry[constant.ENTRY_KEY][constant.ANSWERS_PAGES_KEY][page_k][answer_k] = \
                            f"gs://{config.IMAGE_STORE_BUCKET}/{file_store_path}"
                    else:
                        logging.error(
